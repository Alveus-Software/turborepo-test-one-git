...
export default async function ConfigurationPage() {
  ...
  const { parent, children } = await getModuleWithChildren("website");
  ...
  const filteredChildren = children.filter((child) => {
    if (child.code === "logo" && !userPermissions.includes("menu:logo")) {
      return false; 
    }

    if (child.code === "number_whatsapp" && !userPermissions.includes("menu:number_whatsapp")) {
      return false; 
    }

    if (child.code === "social-media" && !userPermissions.includes("menu:social-media")) {
      return false; 
    }
            ...
            {filteredChildren.map((child) => {
              ...
              return (
                <Link
                  ...
                  className="group"
                >
                  <div className="bg-custom-bg-secondary rounded-2xl p-6 border border-custom-border-secondary hover:bg-custom-bg-hover hover:border-custom-accent-border hover:shadow-lg hover:shadow-custom-accent transition-all duration-200 h-full flex flex-col relative">
