...
import { MeasurementList } from "@repo/components/dashboard/unidades-medida/measurement-list";
import { Suspense } from "react";
import { getUserWithPermissions } from "@repo/lib/actions/user.actions";
import { CategoryListSkeleton } from "@repo/components/dashboard/categorias/category-list-skeleton";
...
export default async function measurementsPage() {
  ...
  const canReadProducts = permissions.includes("read:measurement");
  const canCreateCategory = permissions.includes("create:measurement");
    ...
    <div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex justify-between items-center">
          ...
          {canCreateCategory && (
            <Link
              href="/dashboard/productos/unidades-medida/crear"
            ...
            >
              ...
              <span className="hidden md:inline">Crear Unidad</span>
            </Link>
          )}
        </div>
        ...
        {canReadProducts ? (
          <Suspense fallback={<CategoryListSkeleton />}>
            <MeasurementList userPermissions={permissions} />
          </Suspense>
        ) : (
          <div className="bg-white rounded-lg shadow-sm p-6 text-center text-red-600">
            No tienes permisos para ver las las unidades de medida.
          </div>
        )}
      </div>
    </div>
