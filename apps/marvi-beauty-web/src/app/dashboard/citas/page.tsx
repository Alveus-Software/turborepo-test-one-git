...
export default async function CitasParentPage() {
  ...
  const { parent, children } = await getModuleWithChildren("appointments_parent");
    ...
    <div className="min-h-screen bg-custom-bg-primary p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {filteredChildren.length > 0 ? (
            ...
            {filteredChildren.map((child) => {
              ...
              return (
                <Link
                  ...
                  className="group"
                >
                  <div className="bg-custom-bg-tertiary rounded-2xl p-6 border border-custom-border-secondary hover:bg-[#E3E2DD] hover:border-custom-accent-primary/40 hover:shadow-xl hover:shadow-custom-accent-primary/5 transition-all duration-200 h-full flex flex-col relative">
                    ...
                    {/* Ícono */}
                    <div className="mb-4">
                      <div className="inline-flex p-3 bg-[#E3E2DD] rounded-xl group-hover:bg-custom-accent-primary/10 transition-colors">
                        <ChildIcon className="w-7 h-7 text-custom-accent-primary group-hover:text-custom-accent-primary transition-colors" />
                      </div>
                    </div>
          ...
          <div className="bg-custom-bg-tertiary rounded-2xl p-12 border border-custom-border-secondary text-center">
            <FolderOpen className="w-16 h-16 text-custom-text-muted mx-auto mb-4" />
            <p className="text-custom-text-disabled">No hay módulos disponibles</p>
          </div>
        )}
      </div>
    </div>
