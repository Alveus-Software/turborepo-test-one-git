...
// Estilos optimizados para ticket de pollería (80mm)
...
const OrderComandaPDF = ({ order }: OrderComandaPDFProps) => {
  ...
  const restaurantInfo = {
    ...
    phone: "Tel: +52 311-198-2683",
    website: "polleriagris.com",
  }
    ...
    <Document>
      <Page size={[226.77, 841.89]} style={styles.page}>
        ...
        <View style={styles.orderInfo}>
          <Text style={styles.orderNumber}>TICKET DE COMPRA</Text>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.addressBox}>
            {order.customer_name && <Text style={styles.multiLineText}>Nombre: {order.customer_name}</Text>}
            {order.customer_phone && <Text style={styles.multiLineText}>Teléfono: {order.customer_phone}</Text>}
          </View>
        </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.addressBox}>
              <Text style={styles.multiLineText}>
                ...
                {order.address.interior_number ? `, Int. ${order.address.interior_number}` : ""}
              </Text>
              <Text style={styles.multiLineText}>{order.address.neighborhood}</Text>
              <Text style={styles.multiLineText}>CP: {order.address.postal_code}</Text>
            </View>
          </View>
        )}
            ...
            <View key={item.id} style={styles.productRow}>
              <View style={styles.productMainInfo}>
                <View style={styles.productDescription}>
                  ...
                  <Text style={styles.productDetails}>{formatCurrency(item.product_price)} c/u</Text>
                </View>
                <Text style={styles.productQuantity}>{item.quantity}</Text>
                <Text style={styles.productPrice}>{formatCurrency(item.quantity * item.product_price)}</Text>
              </View>
            </View>
          ))}
        </View>
          ...
          <View style={styles.section}>
            ...
            {order.special_instructions && (
              <View style={styles.instructions}>
                <Text style={{ fontWeight: "bold", fontSize: 6, marginBottom: 1 }}>Instrucciones del cliente:</Text>
            ...
            {order.delivery_instructions && (
              <View style={styles.instructions}>
                <Text style={{ fontWeight: "bold", fontSize: 6, marginBottom: 1 }}>Instrucciones de entrega:</Text>
        ...
        <View style={styles.footer}>
          ...
          <Text>Ticket generado el {new Date().toLocaleString("es-MX")}</Text>
          <Text>
            {restaurantInfo.website} • {restaurantInfo.phone}
          </Text>
        </View>
      </Page>
    </Document>
