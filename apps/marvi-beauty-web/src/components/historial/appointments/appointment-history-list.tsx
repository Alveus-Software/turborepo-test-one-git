...
import { getMyAppointmentHistory } from "@repo/lib/actions/appointment.actions";
import type { AppointmentsResponse } from "@repo/lib/actions/appointment.actions";
...
export function AppointmentHistoryList() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between mb-3">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium
                       text-custom-text-primary bg-custom-bg-primary border border-gray-700 rounded-lg
                       hover:bg-custom-bg-secondary transition-colors"
          >
            <Filter className="w-4 h-4" />
            Filtros
            {hasActiveFilters && (
              <span className="inline-flex items-center justify-center w-5 h-5
                               text-xs font-semibold text-custom-text-primary bg-custom-bg-primary rounded-full">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-1.5 text-xs
                         font-medium text-custom-text-primary hover:bg-gray-700 rounded-md"
            >
              <X className="w-3 h-3" />
              Limpiar todo
            </button>
          ...
          <div className="bg-custom-bg-primary
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="space-y-2 lg:col-span-3">
                <Label className="text-xs font-semibold text-custom-text-primary uppercase tracking-wide">
                  Estado
                </Label>
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1 rounded-full text-xs border transition-colors ${
                          active
                            ? "bg-custom-bg-primary border-blue-500 text-custom-text-primary"
                            : "bg-custom-bg-secondary border-gray-700 text-custom-text-secondary hover:bg-gray-700"
                        }`}
                      >
                        {s.label}
                      </button>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-10 bg-custom-bg-primary border-gray-700 text-custom-text-primary"
                />
              </div>
      ...
      {/* CONTENIDO */}
      {loading && <div className="text-custom-text-primary py-8 text-center">Cargando citas…</div>}

      {!loading && data.appointments.length === 0 && (
        <div className="text-custom-text-primary py-8 text-center border border-gray-800 rounded-lg">
          {hasActiveFilters
