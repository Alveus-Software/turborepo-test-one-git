...
import { renderEmail, sendEmail } from "@repo/lib/actions/send-email";
...
export async function createAvailableAppointment(
  ...
  if (isNaN(appointmentDate.getTime())) {
    return { success: false, message: "Fecha y hora inválidas" };
  }

  // Verificar que sea futura (en UTC)
  ...
  const { error } = await supabase.from("appointments").insert({
    appointment_datetime: appointmentDateUTC,
    notes: payload.notes ?? null,
    status_id: "517e3cc0-0763-4fd0-9195-756fe4617706",
    created_by: user.id,
  });
  ...
  if (error) {
    if (error.code === "23505") {
      return {
        success: false,
        message: "Ya existe un espacio registrado para esa fecha y hora",
      };
    }
    ...
    if (result.client_email && result.appointment_datetime) {
      try {
        ...
        const { sendEmail, renderEmail } =
          await import("@repo/lib/actions/send-email");
...
export async function bookAppointment(
  ...
  try {
    ...
    try {
      const { sendEmail } = await import("@repo/lib/actions/send-email");
      const { format } = await import("date-fns");
      const { es } = await import("date-fns/locale");
