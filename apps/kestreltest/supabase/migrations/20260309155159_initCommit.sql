...
CREATE OR REPLACE FUNCTION public.check_user_permission(user_uuid uuid, permission_code text)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$DECLARE
...
BEGIN
    SELECT EXISTS (
        ...
        JOIN permissions_profiles pp ON p.id = pp.id_profile
        JOIN permissions per ON pp.id_permission = per.id
        WHERE u.id = user_uuid
          AND per.code = permission_code
          AND u.active = true
          AND p.active = true
          AND per.active = true
          AND pp.active = true
...
with check (true);
  ...
  for update
  to authenticated
using (public.check_user_permission(auth.uid(), 'update:slug-code'::text))
with check (public.check_user_permission(auth.uid(), 'update:slug-code'::text));
...
with check ((( SELECT auth.uid() AS uid) = id));
  ...
  for update
  to public
using (public.check_user_permission(auth.uid(), 'update:users'::text))
with check (public.check_user_permission(auth.uid(), 'update:users'::text));
