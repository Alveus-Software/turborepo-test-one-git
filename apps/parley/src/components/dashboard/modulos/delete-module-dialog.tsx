...
export function DeleteModuleDialog({
  ...
  const isConfirmed = confirmationInput.trim() === moduleCode.trim();
      ...
      <DialogContent className="sm:max-w-md bg-white border border-[#f5efe6]">
        <DialogHeader>
          ...
          <DialogDescription className='mt-4 text-sm text-neutral-600'>
            Esta acción no se puede deshacer. El módulo será eliminado permanentemente del sistema.
            <br /><br />
            Escriba el código{' '}
            <span className="font-medium text-[#c6a365]">
              {moduleCode}
            </span>{' '}
            para confirmar la eliminación del módulo <strong>{moduleName}</strong>.
          </DialogDescription>
        </DialogHeader>
        
        <div className="mb-4">
          <Input
            placeholder={`Escriba "${moduleCode}" para confirmar`}
