...
export function DeleteContactGroupDialog({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Llamar a la función server para eliminar el grupo de contacto
      ...
      if (result.success) {
        ...
        onDelete(contactGroupId) // actualizar la lista en el frontend
        onOpenChange(false)
      ...
      <DialogContent className="sm:max-w-md bg-white border border-[#f5efe6]">
        <DialogHeader>
          ...
          <DialogDescription className="mt-4 text-sm text-neutral-600">
            Esta acción no se puede deshacer. El grupo de contacto será eliminado permanentemente del sistema.
            <br /><br />
            Escriba el nombre{" "}
