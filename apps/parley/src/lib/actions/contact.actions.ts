...
/**
 * Obtiene todos los grupos de contactos activos
 */
export async function getAllActiveContactGroups(): Promise<ContactGroup[]> {
  const supabase = await createClient();
...
/**
 * Obtiene contactos que pertenecen a uno o más grupos específicos
 */
export async function getContactsByGroupIds(
  groupIds: string[],
  page = 1,
  pageSize = 50,
  searchQuery = "",
  ...
  try {
    // Primero obtener los contact_ids que están en los grupos seleccionados
    const { data: groupContactsData, error: groupContactsError } =
      await supabase
        .from("group_contacts")
        .select("id_contacts")
        .in("id_contact_groups", groupIds)
        .eq("active", true)
        .is("deleted_at", null);

    if (groupContactsError) {
      console.error(
        "Error al obtener contactos de grupos:",
        groupContactsError,
      );
      return {
        contacts: [],
    ...
    // Si no hay contactos en los grupos seleccionados, retornar vacío
    if (!groupContactsData || groupContactsData.length === 0) {
      return {
        contacts: [],
        total: 0,
        page,
    ...
    // Extraer los IDs únicos de contactos
    const contactIds = Array.from(
      new Set(groupContactsData.map((item) => item.id_contacts)),
    );

    // Ahora obtener los detalles de los contactos
    let query = supabase
      .from("contacts")
      .select(
        `
    ...
    console.error("Error inesperado al obtener contactos por grupos:", error);
    return {
      contacts: [],
      total: 0,
      page,
