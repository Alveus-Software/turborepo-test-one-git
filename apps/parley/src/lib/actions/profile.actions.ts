...
export async function updateProfile(
...
): Promise<{ success: true } | { success: false; field: 'code' | 'path' | 'hierarchy'; message: string }> {
  const supabase = await createClient()
  const currentUser = await getCurrentUser();
  const currentUserId = currentUser?.data?.user?.id;
  const cUser = await getUserById(currentUserId!);

  // 1. Obtener el estado actual del perfil en la DB
  const { data: existingProfile, error: fetchError } = await supabase
    .from('profiles')
    .select('is_write_protected, hierarchy')
    .eq('id', profileId)
    .single();

  if (fetchError || !existingProfile) {
    return { success: false, field: 'code', message: 'Perfil no encontrado.' };
  }

  // 2. Obtener la jerarquía del usuario que está editando
  const { data: userProfile } = await supabase
    .from('profiles')
    .select('hierarchy')
    .eq('id', cUser?.profile_id) 
    .single();

  const editorHierarchy = userProfile?.hierarchy ?? 999;

  if (editorHierarchy >= existingProfile.hierarchy) {
    return { 
      success: false, 
      field: 'hierarchy', 
      message: 'No tienes rango suficiente para modificar este perfil (tu jerarquía debe ser menor).' 
    };
  }
  
  // 3. Preparar datos según la protección de escritura
  ...
  if (existingProfile.is_write_protected) {
    // Si está protegido, SOLO permitimos el campo hierarchy
    dataToUpdate.hierarchy = profileData.hierarchy;
  } 

  // 4. Ejecutar la actualización con el objeto filtrado
  const { error } = await supabase
    .from('profiles')
    .update(dataToUpdate) // USAR dataToUpdate, no profileData directamente
    .eq('id', profileId);

  if (error) {
    console.error('Error updating profile:', error)
    if (error.code === '23505' && error.message.includes('profiles_code_active_unique')) {
      return { success: false, field: 'code', message: 'El código ingresado ya se encuentra registrado.' };
    }
    return { success: false, field: 'code', message: 'No se pudo actualizar el perfil.'+dataToUpdate.name };
  }
