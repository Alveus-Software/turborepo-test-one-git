...
export default async function InventoriesPagePackage() {
  ...
  const { parent, children } = await getModuleWithChildren("inventories");
  ...
  const filteredChildren = children.filter((child) => {
    if (child.code === "record" && !userPermissions.includes("menu:record")) {
      return false;
    }
    if (child.code === "inventories_reports" && !userPermissions.includes("menu:inventories_reports")) {
      return false;
    }
    if (child.code === "management" && !userPermissions.includes("menu:management")) {
      return false;
    }
    ...
    <div className="min-h-screen bg-[#04060B] p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {filteredChildren.length > 0 ? (
            ...
            {filteredChildren.map((child) => {
              ...
              return (
                <Link
                  ...
                  className="group"
                >
                  <div className="bg-[#0A0F17] rounded-2xl p-6 border border-gray-800 hover:bg-[#0E1421] hover:border-yellow-400/30 hover:shadow-lg hover:shadow-yellow-400/5 transition-all duration-200 h-full flex flex-col relative">
                    ...
                    {/* Ícono */}
                    <div className="mb-4">
                      <div className="inline-flex p-3 bg-[#070B14] rounded-xl group-hover:bg-yellow-400/10 transition-colors">
          ...
          <div className="bg-[#0A0F17] rounded-2xl p-12 border border-gray-800 text-center">
