...
export default function CreateEntryPagePackage() {
  const router = useRouter();
  const [allProducts, setAllProducts] = useState<ProductWithStock[]>([]); // Todos los productos
  ...
  const [loadingProducts, setLoadingProducts] = useState(true); // Estado para carga inicial de productos

  // Estado para múltiples productos
  const [productEntries, setProductEntries] = useState<ProductEntry[]>([]);

  // Estado del formulario general - AJUSTE FIJO COMO ORIGEN
  const [formData, setFormData] = useState({
    movement_type: "adjustment" as
      | "purchase"
      | "transfer"
      | "adjustment"
      | "return"
  ...
  // Código de la sucursal principal
  const BRANCH_1_CODE = "BRANCH_1";

  // CÓDIGO FIJO PARA LA UBICACIÓN DE AJUSTES
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Guardar información de BRANCH_1
        if (branchLocation) {
          setBranch1Location({
        ...
        // ESTABLECER UBICACIÓN DE AJUSTES COMO ORIGEN FIJO
        const adjustmentsLocation = locationsResponse.find(
          (loc) => loc.code === ADJUSTMENTS_CODE,
        );

        if (adjustmentsLocation) {
          setFormData((prev) => ({
            ...prev,
            from_location: adjustmentsLocation.id,
          }));
        } else {
          console.warn(
            `No se encontró la ubicación con código ${ADJUSTMENTS_CODE}`,
          );
          toast.warning(`No se encontró la ubicación de Ajustes`);
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (!searchTerm.trim()) {
      // Si no hay término de búsqueda, mostrar los primeros 100 productos
      setFilteredProducts(
        allProducts
          .filter(
            (product) =>
      ...
      const filtered = allProducts.filter(
        (product) =>
          (product.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            product.bar_code && product?.bar_code.toLowerCase().includes(searchTerm.toLowerCase()) &&
            !productEntries.some((entry) => entry.product_id === product.id)),
      );
      setFilteredProducts(filtered.slice(0, 100)); // Limitar a 100 resultados
    }
  ...
  const loadBranchStock = async (productId: string, index: number) => {
    try {
      ...
      setProductEntries((prev) =>
        prev.map((entry, i) =>
          i === index
            ? { ...entry, branchStock: stock, loadingStock: false }
            : entry,
        ),
      );
    } catch (error) {
      ...
      setProductEntries((prev) =>
        prev.map((entry, i) =>
          i === index
            ? { ...entry, branchStock: 0, loadingStock: false }
            : entry,
        ),
      );
    }
  ...
  const addProductFromSearch = () => {
    ...
    const newId = (Date.now() + Math.random()).toString(); // ID único
    ...
    setExpandedProducts([...expandedProducts, false]); // Nuevo producto plegado
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.from_location) {
      setError("No se ha configurado la ubicación de origen (Ajustes)");
      setSubmitting(false);
      return;
    }

    if (!formData.to_location) {
      setError("No se ha configurado la ubicación de destino");
      setSubmitting(false);
      return;
    }
    ...
    try {
      const dataToSend = {
        entries, // Ahora enviamos un array de productos
      ...
      const result = await createInventoryEntry(dataToSend);
      ...
      if (result.success) {
        if (result.failedEntries && result.failedEntries.length > 0) {
          ...
          toast.success(
            result.message || "¡Entradas registradas exitosamente!",
          );
        }
        ...
        toast.warning(result.message || "Error al registrar las entradas");
      }
    } catch (err: any) {
      toast.warning(
        err.message || "Error inesperado al registrar las entradas",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  // Obtener la ubicación de destino (Inventario Online)
  const destinationLocation = locations.find(
    (loc) => loc.id === formData.to_location,
  );

  // Obtener la ubicación de origen (Ajustes) para mostrarla
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center sm:items-start sm:pt-8">
            <button
              ...
              onClick={handleSubmit} // Llama directamente a handleSubmit
              disabled={
              ...
              title={submitting ? "Registrando..." : "Registrar entrada"}
            >
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            <div>
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <input
                  type="text"
                  value={originLocation ? originLocation.name : "Ajustes"}
            ...
            {/* Ubicación de Destino (Fija - Inventario Online) */}
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1 sm:mb-2">
                Destino del ingreso *
              </label>
          ...
          <div className="sm:col-span-2">
            ...
            <select
              ...
              disabled={loading}
            >
              <option value="purchase">Compra de proveedor</option>
              <option value="transfer">Transferencia entre ubicaciones</option>
              <option value="adjustment">Ajuste de inventario</option>
              <option value="production">Producción interna</option>
          ...
          <div>
            ...
            <textarea
              rows={2}
              placeholder="Motivo del ajuste, observaciones adicionales..."
            ...
            <div className="flex gap-2 sm:gap-3">
              <div className="flex-1 relative">
                <input
                  ...
                  onBlur={() => {
                    // Pequeño delay para permitir hacer clic en los items
            ...
            <div className="space-y-3 sm:space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-base sm:text-lg font-medium text-white">
                  Productos agregados ({productEntries.length})
                </h3>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm">
                <div>
                  ...
                  <p className="font-medium text-blue-400">
                    {getLocationName(formData.from_location) || "Ajustes"}
                  </p>
                </div>
                <div>
                  <span className="text-gray-400">Hacia:</span>
                  <p className="font-medium text-green-400">
                    {getLocationName(formData.to_location) ||
                      "Inventario Online"}
                  </p>
                </div>
                <div>
