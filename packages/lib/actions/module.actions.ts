...
import { type NewModulePayload } from '../utils/definitions';
import { type ModulesHierarchy } from '../utils/definitions';
...
export async function updateModule(
  ...
  const { error } = await supabase
    .from('modules')
    .update({...moduleData, updated_by: currentUser?.data?.user?.id, updated_at: new Date()})
    .eq('id', moduleId) 
