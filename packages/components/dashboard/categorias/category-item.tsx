...
export default function CategoryItem({
  ...
  return (
    <div className={isChild ? "ml-4 sm:ml-6 mb-3" : "mb-3"}>
      <div className="flex items-start justify-between p-4 rounded-lg bg-white border border-gray-200 shadow-xs hover:shadow-md transition-shadow-sm cursor-pointer gap-4">
        {/* Contenido principal */}
        <div className="flex items-start gap-4 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0 space-y-1 mr-2">
            <div className="flex flex-col sm:flex-row sm:items-start gap-2">
              <h3 className="text-base font-semibold text-gray-900 line-clamp-2 leading-tight flex-1 min-w-0">
                {category.title}
              </h3>
              <span
                className={`px-2 py-1 rounded-full text-xs font-medium w-fit flex-shrink-0 mt-0.5 ${
                  category.active
        ...
        {(canUpdate || canDelete) && (
          <div className="flex items-start flex-shrink-0 pt-1">
            <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
              ...
              <PopoverContent
                className="w-48 p-3 bg-white border border-gray-200 rounded-lg shadow-lg"
                align="end"
              >
